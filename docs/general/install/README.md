...
module "cloudfront-cache-policy" {
    source = "github.com/pbs/terraform-cloudfront-cache-policy-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/cloudfront-cache-policy'
gh -R 'pbs/terraform-cloudfront-cache-policy-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/cloudfront-cache-policy'
rm -f release.tar.gz
```
...
```bash
git remote add -f cloudfront-cache-policy git@github.com:pbs/terraform-cloudfront-cache-policy-module.git
git subtree add --prefix terraform/modules/cloudfront-cache-policy cloudfront-cache-policy main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/cloudfront-cache-policy cloudfront-cache-policy $REF --squash
```
